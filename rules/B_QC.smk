...
# ---------------------------------------------------------------------
# 1. Load metadata and global definitions
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 2. Determine QC version and update output path if needed
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 3. Environment setup (optional marker rule)
# ---------------------------------------------------------------------
rule setup_env:
    output:
    ...
    shell:
        ...
        module purge
        uv venv --python 3.9
        uv pip install venn alphastats adjustText snakemake
        touch {output.env_flag}
        """

# ---------------------------------------------------------------------
# 4. Run AlphaStats QC — per workflow
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 5. Run AlphaStats QC — per cohort
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# 6. Marker rule — All individual AlphaStats analyses completed
# ---------------------------------------------------------------------
rule run_all_QC:
    input:
        expand(f"{data_dir}{{workflow}}/{{workflow}}_corrected.pg_matrix.tsv", workflow=WORKFLOWS),
